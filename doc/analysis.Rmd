...
```{r}
mosaic(returnShipment ~ cut(price, c(10, 20, 30, 40, 50, 60, 1000)),
...
barplot(table(with(dt.train, cut(price, c(0, 1:20 * 10 - 0.001, Inf)))), las=2,
        main="Histogram of Prices (Train)")
barplot(table(with(dt.test, cut(price, c(0, 1:20 * 10 - 0.001, Inf)))), las=2,
        main="Histogram of Prices (Test)")
```
...
```{r}
tail(sort(table(c(dt.train$customerID, dt.test$customerID))), 10)
```
...
On the extremes, they are are not really normally distributed, but very close!
...
plot(rownames(r), r, type="l", main="Ratio of Returned Items vs Age",
     xlab="Age", ylab="Returned Items (Ratio)")
```
...
The last date appears unusually often. Let's plot the number of accounts opened,
neglecting the date that appears unusually often:

```{r}
counts <- head(sort(table(dt.train$creationDate)), -1)
plot(counts, type="l", xaxt="n")
```
