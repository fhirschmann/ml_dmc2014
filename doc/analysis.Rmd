...
```{r, warning=F, message=F}
library(plyr)
dt.merged$week <- ((year(dt.merged$orderDate)-2012)*52 + week(dt.merged$orderDate)) - 13

tbl <- t(sapply(1:57, function(w) prop.table(table(dt.merged[dt.merged$week == w, ]$fewcolors))))

rownames(tbl) <- 1:57
plot(rownames(tbl),tbl[,1], type="l", col="black", ylim=c(0,1))
lines(rownames(tbl), tbl[, 2], col="blue")
lines(rownames(tbl), tbl[, 3], col="brown")
lines(rownames(tbl), tbl[, 4], col="green")
lines(rownames(tbl), tbl[, 5], col="grey")
lines(rownames(tbl), tbl[, 6], col="pink")   #other
lines(rownames(tbl), tbl[, 7], col="red")
lines(rownames(tbl), tbl[, 8], col="orange") #white
lines(rownames(tbl), tbl[, 9], col="yellow")
```

colors are selfexplanatory except for: pink = other, orange = white

We see: blue, red, brown dont change much. Black rises, white/grey falls in cold months. Yellow and 'other' stay very low the whole time. Green rises all the time, which explains the difference in the other barplot.
...
- compare to "innocent" timeperiod
    - what's a good candidate? -> september
