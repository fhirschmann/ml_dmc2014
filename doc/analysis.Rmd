...
```{r, warning=FALSE}
p <- rbind(data.frame(prop.table(table(dt.test$fewcolors)), group="test"), data.frame(prop.table(table(dt.train$fewcolors)), group="train"))

ggplot(p, aes(x=Var1, y=Freq, fill=group)) + geom_bar(position="dodge") + ggtitle("Color Frequencies in the Train and Test Set")

```

roughly the same distribution, green differs.
...
see if itemIDs show up are present at more than on manufacturer:
...
-> 46 manufacturers do appear in the train but not the testset
