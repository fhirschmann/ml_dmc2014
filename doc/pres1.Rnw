...
\begin{frame}{Missing Values}
    ...
    print(xtable(nas[nas$Train > 0 | nas$Test > 0, ], digits=4,
                 caption="Missing values in percent."))
    @
\end{frame}

\begin{frame}{Class Frequency Equality}
    \Large Question: Are class frequencies distributed equally in
    the Train and Test set?
\end{frame}


\begin{frame}{Class Frequency Equality of Train/Test Data}
    <<results="asis">>=
...
\begin{frame}{$\chi^2$-Test of Train/Test Data Frequency}
    <<chisq, results="asis">>=
    ps <- sapply(c("domain", "voucher", "model", "salutation", "paymenttype", "case",
                   "month", "weekday", "deliverytype", "entry"),
