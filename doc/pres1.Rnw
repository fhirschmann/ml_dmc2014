...
\begin{frame}{Predictors}
    ...
    colors <- c()
    colors[setdiff(names(dt), names(dt.c50))] <- "black" # not used
...
\begin{frame}{Remedies for Severe Class Imbalance}
    \begin{minipage}[t]{0.54\columnwidth}
    \begin{itemize}
        \item Imbalance causes some models to always predict \texttt{no}
        \item Several counter-strategies available. Amongst others:
            \begin{itemize}
...
\begin{frame}{Category Freq Distribution}
    \Large Are category frequencies distributed equally in
    the Train and Test set?
\end{frame}

\begin{frame}{Category Freq Distribution: Example}
    <<results="asis">>=
...
\begin{frame}{Category Freq Distribution: $\chi^2$-Test}
    <<chisq, results="asis">>=
...
\begin{frame}{Category Freq Distribution: 0-Probability Cases}
...
\begin{frame}{Category Freq Distribution: advertisingdatacode}
    <<fig.height=5>>=
...
\begin{frame}{Conclusion}
    \begin{itemize}
        \item Understanding data is key (and not necessarily the best algorithm; but in a competition, it actually is)
        \item Do Attribute Selection
        \item Use Domain Specific Knowledge (proximity to point of sale?)
        \item Removing outliers isn't the only strategy available
        \item C5.0 provides reasonable results
