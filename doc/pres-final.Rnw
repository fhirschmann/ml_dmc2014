...
\begin{alltt}
\hlstd{data} \hlkwb{<-} \hlkwd{data.table}\hlstd{(}\hlkwd{read.table}\hlstd{(}\hlstr{"task/orders_train.txt"}\hlstd{,} \hlkwc{sep} \hlstd{=} \hlstr{";"}\hlstd{,} \hlkwc{header} \hlstd{= T))}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\begin{kframe}
\begin{alltt}
\hlkwd{print}\hlstd{(}\hlkwd{xtable}\hlstd{(data[}\hlnum{1}\hlopt{:}\hlnum{10}\hlstd{,} \hlkwd{c}\hlstd{(}\hlstr{"itemID"}\hlstd{,} \hlstr{"customerID"}\hlstd{,} \hlstr{"price"}\hlstd{),} \hlkwc{with}\hlstd{=F]))}
\end{alltt}
\end{kframe}% latex table generated in R 3.0.3 by xtable 1.7-3 package
% Sun Jun 15 14:17:50 2014
\begin{table}[ht]
\centering
\begin{tabular}{rrrr}
  \hline
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{data[,} \hlkwd{`:=`}\hlstd{(orderSumPrices,} \hlkwd{sum}\hlstd{(price)),} \hlkwc{by} \hlstd{=} \hlkwd{c}\hlstd{(}\hlstr{"customerID"}\hlstd{,} \hlstr{"orderDate"}\hlstd{)]}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\begin{tabular}{rlrrr}
  \hline
 & orderDate & customerID & price & orderSumPrices \\ 
  \hline
1 & 2012-04-01 & 794 & 69.90 & 209.80 \\ 
  2 & 2012-04-01 & 794 & 69.95 & 209.80 \\ 
  3 & 2012-04-01 & 794 & 69.95 & 209.80 \\ 
  4 & 2012-04-02 & 808 & 39.90 & 39.90 \\ 
  5 & 2012-04-02 & 825 & 29.90 & 249.70 \\ 
  6 & 2012-04-02 & 825 & 89.90 & 249.70 \\ 
  7 & 2012-04-02 & 825 & 129.90 & 249.70 \\ 
  8 & 2012-04-02 & 850 & 21.90 & 533.30 \\ 
  9 & 2012-04-02 & 850 & 21.90 & 533.30 \\ 
  10 & 2012-04-02 & 850 & 39.90 & 533.30 \\ 
   \hline
\end{tabular}
\end{table}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{data}\hlopt{$}\hlstd{priceDiscrete} \hlkwb{<-} \hlkwd{cut}\hlstd{(data}\hlopt{$}\hlstd{price,} \hlkwd{c}\hlstd{(}\hlnum{0}\hlstd{,} \hlnum{1}\hlopt{:}\hlnum{20} \hlopt{*} \hlnum{10}\hlstd{,} \hlnum{Inf}\hlstd{),} \hlkwc{left} \hlstd{= T,} \hlkwc{right} \hlstd{= F)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{orderDates} \hlkwb{<-} \hlkwd{unique}\hlstd{(data[,} \hlkwd{c}\hlstd{(}\hlstr{"customerID"}\hlstd{,} \hlstr{"orderDate"}\hlstd{),} \hlkwc{with} \hlstd{= F])}
\hlkwd{setkeyv}\hlstd{(orderDates,} \hlkwd{c}\hlstd{(}\hlstr{"customerID"}\hlstd{,} \hlstr{"orderDate"}\hlstd{))}
\hlstd{orderDates[,} \hlkwd{`:=`}\hlstd{(orderDatePrev,} \hlkwd{as.Date}\hlstd{(}\hlkwd{c}\hlstd{(}\hlnum{NA}\hlstd{,} \hlkwd{head}\hlstd{(orderDate,} \hlopt{-}\hlnum{1}\hlstd{)))),} \hlkwc{by} \hlstd{=} \hlkwd{c}\hlstd{(}\hlstr{"customerID"}\hlstd{)]}
\hlstd{orderDates}\hlopt{$}\hlstd{orderTimePrev} \hlkwb{<-} \hlstd{orderDates}\hlopt{$}\hlstd{orderDatePrev} \hlopt{-} \hlstd{orderDates}\hlopt{$}\hlstd{orderDate}
\end{alltt}
\end{kframe}
\end{knitrout}

% latex table generated in R 3.0.3 by xtable 1.7-3 package
% Sun Jun 15 14:18:11 2014
\begin{table}[ht]
\centering
\begin{tabular}{rllr}
  \hline
customerID & orderDate & orderDatePrev & orderTimePrev \\ 
  \hline
  6 & 2012-04-19 &  &  \\ 
    6 & 2012-04-25 & 2012-04-19 &  -6 \\ 
    6 & 2012-09-23 & 2012-04-25 & -151 \\ 
    6 & 2012-10-06 & 2012-09-23 & -13 \\ 
    6 & 2013-01-11 & 2012-10-06 & -97 \\ 
    9 & 2012-08-01 &  &  \\ 
    9 & 2012-08-10 & 2012-08-01 &  -9 \\ 
    9 & 2012-12-31 & 2012-08-10 & -143 \\ 
    9 & 2013-01-09 & 2012-12-31 &  -9 \\ 
    9 & 2013-01-18 & 2013-01-09 &  -9 \\ 
   \hline
\end{tabular}
\end{table}
\end{frame}
...
\begin{frame}{Feature Engineering: Feature Constuction}
	\begin{tabular}{c l}
		Type\\
		{\usebeamercolor[fg]{structure} \textbf{I}} & features computed over the entire data set (unsupervised)\\
    ...
    {\usebeamercolor[fg]{structure} \textbf{III}} & features computed based on ALL available data and added\\
			& to another set (unsupervised)\\[0.5em]
			& \begin{tikzpicture}[scale=0.4]
					\fill[red,opacity=0.7] (0,0) ellipse (1 and 0.55);
...
\begin{frame}{Feature Engineering: Feature Constuction Type II}
	...
	\end{block}
		...
		\begin{tabular}{rrrrl}
			...
			2 & 2657 & 40 & no & 0.6064909\\
			3 & 2387 & 5+ & yes & 0.5200000\\
...
\begin{frame}{Feature Engineering: Lessons Learned}
	\begin{itemize}
		...
		\item \textbf{Case Studies}:\\
			e.g. customer is having an order for one$\mid$some item(s) having same$\mid$different sizes with same$\mid$different color(s) 
		\item \textbf{What-if scenarios}:\\
			e.g. If item doesn't$\mid$does fit but customer likes$\mid$dislikes the design, what does he do?
...
\begin{frame}{Data Sampling}
    something about our sampling work
\end{frame}
