...
if (length(args) > 0) {
    ...
    build <- c("BAG_01", "BAG_02","BAG_03","BAG_04","BAG_05","BAG_06",
               "BAG_07","BAG_08","BAG_09","BAG_10","BAG_11","BAG_12",
               "BAG_13","BAG_14","BAG_15","BAG_16","BAG_17","BAG_18",
               "BAG_19","BAG_20")
}
for (i in build) {
    dt.dmc.ids$train[[paste(gsub("BAG_","B",i),"0",sep="")]] <- as.numeric(as.character(read.csv(paste("eva/", i, "_M3_train.txt", sep=""))$orderItemID))
    dt.dmc.ids$test[[paste(gsub("BAG_","B",i),"0",sep="")]] <- as.numeric(as.character(read.csv(paste("eva/M30_test.txt", sep=""))$orderItemID))
}

for (i in build) {
  dt.dmc.ids$train[[paste(gsub("BAG_","B",i),"1",sep="")]] <- as.numeric(as.character(read.csv(paste("eva/", i, "_M3_train.txt", sep=""))$orderItemID))
  dt.dmc.ids$test[[paste(gsub("BAG_","B",i),"1",sep="")]] <- as.numeric(as.character(read.csv(paste("eva/M31_test.txt", sep=""))$orderItemID))
}
...
saveRDS(dt.train, file="bagdata.train.RData")
saveRDS(dt.test, file="bagdata.test.RData")
saveRDS(dt.dmc, file="bagdata.dmc.RData")
saveRDS(dt.dmc.ids, file="bagdata.dmc.ids.RData")
#saveRDS(na, file="bagna.RData")