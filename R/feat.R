...
add.features.otf <- function(to, from) {
    ...
    ###price features##########################################################################
    temp <- rbind(data.table(dt.train)[,list(itemID,price,orderDate,size,color)], data.table(dt.test)[,list(itemID,price,orderDate,size,color)]) 
    
    #1,2 ->min and max price
    temp<-data.table(temp)
    itemPriceRange<- temp[,list(min(price),max(price)),by=itemID]
    ...
    itemPriceRange[,priceRange:="no"] #hinzufügen Feature priceRange
    itemPriceRange[max!=min,priceRange:="yes"] #0:keine Range       1:Range
    ...
    dt.from<-merge(dt.from,itemPriceRange,x.all=TRUE)
    setkey(dt.to,itemID)
    dt.to<-merge(dt.to,itemPriceRange,x.all=TRUE)
    ...
    dt.to<-merge(dt.to,temp,x.all=TRUE)
    ...
    ###itemID cutting##########################################################################
    # deletes all itemID from train and test set not intersecting
    
    temp1<-dt.from[,length(customerID),by=itemID]
    temp1[,V1:=NULL]
    setkey(temp1,itemID)
    temp1<-merge(temp1,dt.to[,length(customerID),by=itemID], all.x=TRUE)
    setnames(temp1,c("V1"),c("itemIDnew"))
    
    temp2<-dt.to[,length(customerID),by=itemID]
    temp2[,V1:=NULL]
    setkey(temp2,itemID)
    temp2<-merge(temp2,dt.from[,length(customerID),by=itemID], all.x=TRUE)
    setnames(temp2,c("V1"),c("itemIDnew"))
    
    setkey(dt.from,itemID)
    dt.from<-dt.from[temp1]
    dt.from$itemID[is.na(dt.from$itemIDnew)] <- NA
    dt.from[,itemIDnew:=NULL]
    
    setkey(dt.to,itemID)
    dt.to<-dt.to[temp2]
    dt.to$itemID[is.na(dt.to$itemIDnew)] <- NA
    dt.to[,itemIDnew:=NULL]
    rm(temp1)
    rm(temp2)
    
    ###manufacturerID cutting##########################################################################
    # deletes all IsD from train and test set not intersecting
    
    temp1<-dt.from[,length(customerID),by=manufacturerID]
    temp1[,V1:=NULL]
    setkey(temp1,manufacturerID)
    temp1<-merge(temp1,dt.to[,length(customerID),by=manufacturerID], all.x=TRUE)
    setnames(temp1,c("V1"),c("manufacturerIDnew"))
    
    temp2<-dt.to[,length(customerID),by=manufacturerID]
    temp2[,V1:=NULL]
    setkey(temp2,manufacturerID)
    temp2<-merge(temp2,dt.from[,length(customerID),by=manufacturerID], all.x=TRUE)
    setnames(temp2,c("V1"),c("manufacturerIDnew"))
    
    setkey(dt.from,manufacturerID)
    dt.from<-dt.from[temp1]
    dt.from$manufacturerID[is.na(dt.from$manufacturerIDnew)] <- NA
    dt.from[,manufacturerIDnew:=NULL]
    
    setkey(dt.to,manufacturerID)
    dt.to<-dt.to[temp2]
    dt.to$manufacturerID[is.na(dt.to$manufacturerIDnew)] <- NA
    dt.to[,manufacturerIDnew:=NULL]
    rm(temp1)
    rm(temp2)
    
    ###customerID cutting##########################################################################
    # deletes all IDs from train and test set not intersecting
    
    temp1<-dt.from[,length(customerID),by=customerID]
    temp1[,V1:=NULL]
    setkey(temp1,customerID)
    temp1<-merge(temp1,dt.to[,length(customerID),by=customerID], all.x=TRUE)
    setnames(temp1,c("V1"),c("customerIDnew"))
    
    temp2<-dt.to[,length(customerID),by=customerID]
    temp2[,V1:=NULL]
    setkey(temp2,customerID)
    temp2<-merge(temp2,dt.from[,length(customerID),by=customerID], all.x=TRUE)
    setnames(temp2,c("V1"),c("customerIDnew"))
    
    setkey(dt.from,customerID)
    dt.from<-dt.from[temp1]
    dt.from$customerID[is.na(dt.from$customerIDnew)] <- NA
    dt.from[,customerIDnew:=NULL]
    
    setkey(dt.to,customerID)
    dt.to<-dt.to[temp2]
    dt.to$customerID[is.na(dt.to$customerIDnew)] <- NA
    dt.to[,customerIDnew:=NULL]
    rm(temp1)
    rm(temp2)
...
add.features.otf <- function(to, from) {
    ...
    #join itemPriceRange an itemID  -> adding features to FROM and TO
    setkey(dt.from,itemID)
    dt.from<-merge(dt.from,itemPriceRange,x.all=TRUE)
    setkey(dt.to,itemID)
    dt.to<-merge(dt.to,itemPriceRange,x.all=TRUE)
    #Adding two other attributes
    #6 -> actual discount on piece
    dt.from[, actDiscount := round(1-(price/max),digits=2)*100]
    dt.to[, actDiscount := round(1-(price/max),digits=2)*100]
    #7 -> boolean discounted TRUE False
    ...
    #3,4
    setnames(temp,c("V1", "V2"),c("custItemsReturned","custItemsOrdered"))
    #join die temp an customerID  -> adding rating to FROM and TO
    setkey(dt.from,customerID)
    dt.from<-dt.from[temp]
    setkey(dt.to,customerID)
    dt.to<-merge(dt.to,temp,x.all=TRUE)
    ...
    #3,4
    setnames(temp,c("V1", "V2"),c("itemsReturned","itemsOrdered"))
    #join der temp an itemID -> adding rating to FROM and TO
    setkey(dt.from,itemID)
    dt.from<-dt.from[temp]
    setkey(dt.to,itemID)
    dt.to<-merge(dt.to,temp,x.all=TRUE)
    ...
    #3,4
    setnames(temp,c("V1", "V2"),c("manuItemsReturned","manuItemsOrdered"))
    #join temp an manufacturerID -> adding rating to FROM and TO
    setkey(dt.from,manufacturerID)
    dt.from<-dt.from[temp]
    setkey(dt.to,manufacturerID)
    dt.to<-merge(dt.to,temp,x.all=TRUE)
    ...
    ###itemID cutting##########################################################################
    # deletes all itemIDs from train and test set not intersecting
